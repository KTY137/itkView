...
Ohne den Schalter bleibt der Launcher beim absichtlich unerreichbaren Testziel.
Ein DNS-Fehler fuer `itkpd-test.unicorncollege.cz` ist in diesem Modus daher
erwartbar; fuer einen echten Sync muss der Production-Read-Schalter gesetzt sein.
...
erhalten. Ein beim Serverstart noch aktiver Job wird als `interrupted`
geschlossen und nicht automatisch mit einem moeglicherweise anderen Snapshot
fortgesetzt.
